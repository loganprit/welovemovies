import type { ApiError, ApiRequestOptions, ApiResponse, Movie, Review, Theater } from "$lib/types/api";

const DEFAULT_API_URL = "http://localhost:5001";
const apiBaseUrl = (import.meta.env.PUBLIC_API_URL || import.meta.env.VITE_API_URL || DEFAULT_API_URL).replace(/\/$/, "");
...
async function fetchJson<T>(url: string | URL, options: ApiRequestOptions, onCancel: T): Promise<T> {
  try {
...
async function populateReviews(movie: Movie, signal?: AbortSignal): Promise<Movie> {
...
async function populateTheaters(movie: Movie, signal?: AbortSignal): Promise<Movie> {
...
export function listMovies(options: ListMoviesOptions = {}): Promise<Movie[]> {
  ...
  if (!moviesCache.has(cacheKey)) {
    ...
    moviesCache.set(
      cacheKey,
      fetchJson<Movie[]>(url, { headers, signal }, []).then((movies) =>
...
export function listTheaters(signal?: AbortSignal): Promise<Theater[]> {
  theatersCache ??= fetchJson<Theater[]>(`${apiBaseUrl}/theaters`, { headers, signal }, []);
  return theatersCache;
}

export function readMovie(movieId: number, signal?: AbortSignal): Promise<Movie> {
  if (!movieCache.has(movieId)) {
    movieCache.set(
      movieId,
      fetchJson<Movie>(`${apiBaseUrl}/movies/${movieId}`, { headers, signal }, {} as Movie)
...
export async function readTheater(theaterId: number, signal?: AbortSignal): Promise<Theater | undefined> {
...
export async function deleteReview(reviewId: number): Promise<void> {
  await fetchJson<void>(`${apiBaseUrl}/reviews/${reviewId}`, { method: "DELETE", headers }, undefined);
  movieCache.clear();
  moviesCache.clear();
}

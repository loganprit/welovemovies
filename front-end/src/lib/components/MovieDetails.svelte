<script lang="ts">
  ...
  let { movie, variant = "list" }: { movie: Movie; variant?: "list" | "full" } =
    $props();
...
>
  ...
  <div class="flex-1 space-y-6">
    <h3
      class={`${isList ? "text-3xl font-bold tracking-tight" : "font-poppins-heading text-3xl"} ${$theme === "dark" ? "text-white" : "text-gray-900"}`}
    >
      {movie.title}
    </h3>
    <p
      class={`${isList ? "" : "leading-relaxed"} ${$theme === "dark" ? "text-gray-300" : "text-gray-700"}`}
    >
      {movie.description}
    </p>
    <div class="space-y-3">
      <p
        class={`flex items-center gap-2 ${$theme === "dark" ? "text-gray-200" : "text-gray-800"}`}
      >
        <span class="font-medium">Runtime:</span>
        <span
          class={`rounded-full px-3 py-1 ${$theme === "dark" ? "bg-primary-900/20 text-primary-300" : "bg-primary-50 text-primary-700"}`}
        >
    ...
    {#if isList}
      <a
        ...
        class={`inline-block rounded-lg px-6 py-3 shadow-md transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${
          $theme === "dark"
            ? "bg-primary-500 text-gray-100 hover:bg-primary-400 hover:text-white"
            : "bg-primary-600 text-black hover:bg-primary-700 hover:text-gray-900"
        }`}
